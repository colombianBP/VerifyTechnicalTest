...
class credentials:
    ...
    def write_output(self,zipfilepath='documents_to_process.zip',outfilepath='out'):
        #check if zip file exists
        if not os.path.isfile(zipfilepath):
            raise Exception('Zip file not found in path')

        
        veryfi_client = Client(self.client_id, self.client_secret, self.username, self.api_key)
        response = veryfi_client.process_document(zipfilepath)
        ...
        vendor_names=give_vendor_names(docs,kw1)
        
        kw2='AS SHOWN BELOW'
        kw3='SHIPMENT TO ARRIVE NOT LATER THAN'
        
        shippinngDeets=detailSubset(docs,kw2,kw3)
        ...
        #kw for determining if shipping details are available or
        #atttached in another document
        kw4='ATTACH'
        
        for i in shippinngDeets:
            if i!=-1:
                proc=giveshipping(i,kw4)
        ...
        line_items=[startitem(i,kw5,kw6) for i in docs]
        ...
        writeResults(file_name_list,vendor_names,bill_to_names,bill_to_address,
                ship_to_names,ship_to_address,line_items,outfilepath,tolerance=4)
...
# string ist or -1
def give_vendor_names(docs,kw1):
    vendor_names=[]
    for i in docs:
...
#sup7: Shipping details are not included if kw4 'ATTACH' is included in the description



# determine if 'ATTACH' is present document, 
# returns boolean
def isattached(doc,kw4):
    for i in doc:
        for j in i:
            if kw4 in j.upper():
                return(True)
...
def breakShipping(doc,kw4):
    attached=isattached(doc,kw4)
    counter=0
    bills=[]
    ships=[]
    for i in doc:
...
def giveshipping(doc,kw4):
    breaked=breakShipping(doc,kw4)
...
def writeResults(file_name_list,vendor_names,bill_to_names,bill_to_address,
                ship_to_names,ship_to_address,line_items,outfilepath,tolerance=4):
    ...
    for i in range(len(file_name_list)):
        ...
        with(open(os.path.join(outfilepath,filename),'w') as file):
